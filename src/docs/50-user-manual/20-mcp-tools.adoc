...
Iterates through all documentation sections, passing each one together with the question and previous findings to the LLM. The LLM decides relevance — no keyword matching is used. More accurate than RAG (synonyms and natural language work correctly) but takes a few seconds since each section requires a separate LLM call.
...
| `max_sections` | int \| null | null | Limit sections to check (default: all)
|===
